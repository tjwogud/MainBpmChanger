...
namespace MainBpmChanger
{
    public static class Reflections
    {
        ...
        {
            ParameterExpression instanceExp = Expression.Parameter(type, "instance");
            MemberExpression fieldExp = Expression.Field(field.IsStatic ? null : instanceExp, field);
            Delegate getter = Expression.Lambda(fieldExp, instanceExp).Compile();
        ...
        private static Delegate CreatePropertyGetter(Type type, PropertyInfo property)
        {
            ...
            ParameterExpression instanceExp = Expression.Parameter(type, "instance");
            MethodCallExpression methodExp = Expression.Call(getMethod.IsStatic ? null : instanceExp, getMethod);
            Delegate getter = Expression.Lambda(methodExp, instanceExp).Compile();
        ...
        private static Delegate CreateFieldSetter(Type type, FieldInfo field)
        {
            ...
            Delegate setter = Expression.Lambda(assignExp, instanceExp, valueExp).Compile();
        ...
        private static Delegate CreatePropertySetter(Type type, PropertyInfo property)
        {
            ...
            MethodCallExpression methodExp = Expression.Call(setMethod.IsStatic ? null : instanceExp, setMethod, valueExp);
            Delegate setter = Expression.Lambda(methodExp, instanceExp, valueExp).Compile();
